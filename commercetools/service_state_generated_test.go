...
func TestGeneratedStateGetWithKey(t *testing.T) {
	responseData := ` {
	  "id": "7c2e2694-aefe-43d7-888e-6a99514caaca",
	  "version": 1,
	  "key": "Initial",
	...
	client, server := testutil.MockClient(t, responseData, nil, nil)
	defer server.Close()
	state, err := client.StateGetWithKey(context.TODO(), "dummy-id")
	if err != nil {
		t.Fatal(err)
	}
	assert.NotNil(t, state)
...
func TestGeneratedStateDeleteWithKey(t *testing.T) {
	responseData := ` {
	  "id": "7c2e2694-aefe-43d7-888e-6a99514caaca",
	  "version": 1,
	  "key": "Initial",
	...
	client, server := testutil.MockClient(t, responseData, nil, nil)
	defer server.Close()
	state, err := client.StateDeleteWithKey(context.TODO(), "dummy-id", 1)
	if err != nil {
		t.Fatal(err)
	}
	assert.NotNil(t, state)
