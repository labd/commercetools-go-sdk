...
// UnmarshalJSON override to map the shipping rate input type to the corresponding struct.
...
// MessagesConfiguration is used to configure the Messages feature for the project.
type MessagesConfiguration struct {
	// When true the creation of Messages is enabled.
	Enabled bool `json:"enabled"`
}

// CartValue is used when the shipping rate maps to the sum of the line item prices.
// The value of the cart is used to select a tier. If chosen, it is not possible
// to set a value for the shippingRateInput on the cart. Tiers contain the centAmount
// (a value of 100 in the currency USD corresponds to $ 1.00), and start at 1.
...
// medium, heavy.
// Only a key defined inside the values array can be used to create a tier,
// or to set a value for the shippingRateInput on the cart.
// The keys are checked for uniqueness and the request is rejected if keys are not unique.
...
// CartScore is used when the shipping rate maps to an abstract cart categorization
// expressed through an integer, e.g. shipping score or weight ranges.
...
// ShippingRateInputType can be dynamically selected in three ways.
// The CartValue type uses the sum of all line item prices,
// whereas CartClassification and CartScore use the shipppingRateInput
