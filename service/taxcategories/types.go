...
type TaxRate struct {
	...
	// For countries (e.g. the US) where the total tax is a combination
	// of multiple taxes (e.g. state and local taxes).
	SubRates []SubRate `json:"subRates"`
}

// SubRate is used to calculate the taxPortions field in a cart or order.
// It is useful if the total tax of a country is a combination of multiple
// taxes (e.g. state and local taxes).
type SubRate struct {
	Name string `json:"name"`
	// Number Percentage in the range of [0..1].
	Amount float64 `json:"amount"`
