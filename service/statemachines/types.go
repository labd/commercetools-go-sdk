...
// StateType enumeration contains values for each of the objects that can have
// states assigned.
...
const (
	...
	// LineItemStateType is used by LineItems and CustomLineItems in Carts and
	// Orders
...
const (
	// ReviewIncludedInStatisticsRole is used by Reviews This role only applies
	// for the ReviewState StateType. When the state of a Review has this role,
	// the review will be taken into account for rating statistics.
	ReviewIncludedInStatisticsRole StateRole = "ReviewIncludedInStatistics"

	// ReturnRole is used by Orders in the update action
	// transitionLineItemState. This role only applies for the LineItemState
	// StateType.
	ReturnRole StateRole = "Return"
)

// State allows you to model states of certain objects, such as orders, line
// items, products, reviews, and payments in order to define finite state
// machines reflecting the business logic you'd like to implement.
type State struct {
	ID             string    `json:"id"`
	...
	// A state can be declared as an initial state for any state machine. When a
	// workflow starts, this first state must be an initial state.
	...
	// currently referenced state must have a transition to the new state.
	//  If transitions is an empty list, it means the current state is a final
	// state and no further transitions are allowed. If transitions is not set,
	// the validation is turned off. When performing a transitionState update
	// action, any other state of the same type can be transitioned to.
